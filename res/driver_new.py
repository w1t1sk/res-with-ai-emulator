"""
driver_new.py — AI+RES driver for the NEW emulator (FuXiENSModelV3LatentHierSwin).

Self-contained driver for the QDMC rare event sampling loop using the new
latent-hierarchical-Swin emulator. It provides:
...
All emulator-agnostic machinery (GCM physics, pivotal resampling, checkpoint
I/O, walker lifecycle) is imported from core.py. The only emulator-specific
pieces live here: the model construction in load_model() and the single
inference call `model.sample_prior(x)` inside score_walkers_with_ai().

The companion driver_old.py mirrors this file for the old fuxiens emulator.
"""
...
# Make the new emulator's package importable (src/upd_emulator/...).
_emulator_dir = os.environ.get(
    "AIRES_EMULATOR_DIR", "/home/nishidh/res_aiemulator_sem8/emulator"
)
sys.path.insert(0, os.path.join(_emulator_dir, "src"))
from upd_emulator.models.fuxi_ens_v3_latent_hierswin import FuXiENSModelV3LatentHierSwin


EMULATOR_NAME = "new (FuXiENSModelV3LatentHierSwin)"
...
def load_model(device: torch.device):
    """Load the new latent-hierswin stage-2 checkpoint.

    Reads checkpoint + stats from AIRES_EMULATOR_DIR (default:
    res_aiemulator_sem8/emulator/). The model's architecture hyperparameters
    are read from the checkpoint metadata so the code stays in sync with the
    trained weights.

    Returns
    -------
    model : FuXiENSModelV3LatentHierSwin  (eval mode, on `device`)
    ...
    ckpt_path = os.path.join(
        emulator_dir, "checkpoints", "stage2", "latent_hierswin_rollout_best.pt"
    )
    stats_path = os.path.join(emulator_dir, "resources", "stats", "exac_input_stats.pt")
    ...
    data_mean = stats["physical_mean"].view(1, 14, 1, 1).to(device)
    data_std = stats["physical_std"].view(1, 14, 1, 1).to(device)
    return model, data_mean, data_std


def _emulator_step(model, current_tensor: torch.Tensor) -> torch.Tensor:
    """One emulator rollout step. THE new-emulator-specific inference call."""
    return model.sample_prior(current_tensor)
...
def score_walkers_with_ai(
    ...
    For each walker, M_members stochastic trajectories are drawn from the
    emulator starting from the walker's current (t-1, t) daily-mean state. The
    score is the mean target-region surface temperature averaged over the
    final L-day observable window and across ensemble members.

    Members are processed in microbatches to bound peak GPU memory at large M;
    scores are accumulated online so the full (M, horizon) array is never
    materialised. Walkers without sufficient history or flagged as exploded
    fall back to their recent GCM trajectory temperature.
...
def run_ai_res_scheme(config: dict, scheme: str, target_region_name: str | None = None) -> None:
    """Run a QDMC AI+RES experiment with the new emulator.

    Initialises walkers from the shared aquaplanet spinup, then alternates GCM
    integration chunks with AI-guided pivotal resampling according to the
    chosen C_k schedule. A fault-tolerant checkpoint is saved after every
    resampling step, so an interrupted run resumes where it left off.
    ...
    ----------
    config : dict
        Loaded experiment config (already region-resolvable).
    scheme : str
        Schedule name; must map to a "C_schedule_<scheme>" key in config.
    target_region_name : str, optional
        Override the default target region.
    """
